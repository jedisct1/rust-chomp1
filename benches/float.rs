...
fn match_float(b: &mut Bencher) {
    b.iter(|| ascii::match_float(PI_SLICE))
}
...
fn float_f64(b: &mut Bencher) {
    ...
    b.iter(|| ascii::float::<_, f64>(PI_SLICE))
}
...
fn float_f32(b: &mut Bencher) {
    ...
    b.iter(|| ascii::float::<_, f32>(PI_SLICE))
}
...
fn float_no_conversion(b: &mut Bencher) {
    b.iter(|| ascii::match_float(PI_SLICE).map(|b| b.into_vec()))
}

#[bench]
fn float_small_no_conversion(b: &mut Bencher) {
    b.iter(|| ascii::match_float(&b"1"[..]).map(|b| b.into_vec()))
}
